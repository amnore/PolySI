...
__global__
static void regulateGPU(__half *a, int length) {
  int index = (threadIdx.x + blockIdx.x * blockDim.x) * REGULATE_BATCH;
  //printf("block %d, thread %d, index[%d] => [%f]\n", blockIdx.x, threadIdx.x, index, a[index]);
  for (int i=0; i<REGULATE_BATCH; i++) {
    if (index+i < length) {
      a[index + i] = 2 * ((int)a[index + i] != 0);
    }
  }
}
...
static void regulate(__half *gpu_m, int length, __half *cpu_m) {
  int num_blocks = ceil((double)length/THREADS_PER_BLOCK/REGULATE_BATCH);
  regulateGPU<<<num_blocks, THREADS_PER_BLOCK>>>(gpu_m, length);
  CUDA_ASSERT(cudaGetLastError());
  CUDA_ASSERT(cudaDeviceSynchronize());
...
static int power(__half *cpu_m, int n) {
    static __half *gpu_m = nullptr, *gpu_m2 = nullptr;
    static cublasHandle_t handle_c = nullptr;
    static int matrix_n = 0;
    ...
    if (matrix_n != n) {
        CUDA_ASSERT(cudaFree(gpu_m));
        CUDA_ASSERT(cudaFree(gpu_m2));
        CUDA_ASSERT(cudaMalloc(&gpu_m, n*n*sizeof(__half)));
        CUDA_ASSERT(cudaMalloc(&gpu_m2, n*n*sizeof(__half)));
        matrix_n = n;
    }
  ...
  // (1) copy the matrix to GPU
  CUDA_ASSERT(cudaMemcpy(gpu_m, cpu_m, n*n*sizeof(__half), cudaMemcpyHostToDevice));

  int dense_m = 1;

  // (3.3) dense mm then
  __half *gpu_src = gpu_m;
  __half *gpu_dst = gpu_m2;
  ...
  // (4) copy the result out
  CUDA_ASSERT(cudaMemcpy(cpu_m, gpu_m, n*n*sizeof(__half), cudaMemcpyDeviceToHost));
  std::cerr << "DONE, DM^" << dense_m << "\n";
...
void Java_gpu_GPUmm_booleanPower
  (JNIEnv *env, jclass cls, jobjectArray jmat, jint jn) {
    __half *cpu_mat = new __half[(size_t) jn * jn];
