...
public class SIVerifier<KeyType, ValueType> {
	...
	public boolean audit() {
		...
		System.err.printf("Pruned %d rounds, solved %d constraints\n" + "After prune: graphA: %d, graphB: %d\n", rounds,
				solvedConstraints, graph.getKnownGraphA().edges().size(), graph.getKnownGraphB().edges().size());
		...
		if (!accepted) {
			...
			conflicts.getRight().forEach(c -> System.err.printf("Constraint: %s\n", c));
		}
	...
	private Set<SIConstraint<KeyType, ValueType>> generateConstraints(History<KeyType, ValueType> history,
		...
		history.getEvents().stream().filter(e -> e.getType() == History.EventType.WRITE).forEach(ev -> {
			...
			for (var b : readFrom.successors(a)) {
				for (var key : readFrom.edgeValue(a, b).get()) {
					for (var c : writes.get(key)) {
						...
						constraintEdges.get(Pair.of(a, c)).add(new SIEdge<>(b, c, EdgeType.RW));
					}
				}
			}
		}
		...
		forEachWriteSameKey.accept((a, c) -> constraints
				.add(new SIConstraint<>(constraintEdges.get(Pair.of(a, c)), constraintEdges.get(Pair.of(c, a)))));
	...
	private int pruneConstraints(PrecedenceGraph<KeyType, ValueType> knownGraph,
		...
		var graphA = new MatrixGraph<>(knownGraph.getKnownGraphA(), false);
		var graphB = new MatrixGraph<>(knownGraph.getKnownGraphB(), true);
...
class SISolver<KeyType, ValueType> {
	...
	boolean solve() {
		var lits = Stream.concat(knownLiterals.keySet().stream(), constraintLiterals.keySet().stream())
	 ...
	 *
	 * First construct two graphs: 1. Graph A contains WR, WW and SO edges. 2. Graph
	 * B contains RW edges.
	 *
	 ...
	 *
	 * Literals that are passed as assumptions to monograph: 1. The literals of WR,
	 * SO edges, because those edges always exist. 2. For each constraint, a literal
	 * that asserts exactly one set of edges exist in the graph.
	 */
	SISolver(History<KeyType, ValueType> history, PrecedenceGraph<KeyType, ValueType> precedenceGraph,
		...
		List.of(Pair.of('A', graphA), Pair.of('B', graphB), Pair.of('C', graphC)).forEach(p -> {
			var g = p.getRight();
			var edgesSize = g.edges().stream().map(e -> g.edgeValue(e).get().size()).reduce(Integer::sum).orElse(0);
	...
	private MutableValueGraph<Transaction<KeyType, ValueType>, Set<Lit>> createKnownGraph(
			History<KeyType, ValueType> history, Graph<Transaction<KeyType, ValueType>> knownGraph) {
	...
	private void addConstraints(Set<SIConstraint<KeyType, ValueType>> constraints,
		...
		for (var c : constraints) {
			...
			constraintLiterals
					.put(Logic.or(Logic.and(p1.getLeft(), p2.getRight()), Logic.and(p2.getLeft(), p1.getRight())), c);
		}
	}
}
...
class Utils {
	static <KeyType, ValueType> boolean verifyInternalConsistency(History<KeyType, ValueType> history) {
		...
		Function<History.EventType, Stream<Pair<Integer, History.Event<KeyType, ValueType>>>> getEvents = type -> history
		...
		getEvents.apply(History.EventType.WRITE).forEach(p -> {
			var i = p.getLeft();
			var ev = p.getRight();
			writes.put(Pair.of(ev.getKey(), ev.getValue()), Pair.of(ev.getTransaction(), i));
		});
	...
	static <KeyType, ValueType> MutableValueGraph<Transaction<KeyType, ValueType>, Set<Lit>> composition(
			History<KeyType, ValueType> history, MutableValueGraph<Transaction<KeyType, ValueType>, Set<Lit>> graphA,
			MutableValueGraph<Transaction<KeyType, ValueType>, Set<Lit>> graphB) {
